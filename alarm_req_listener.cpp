...
bool AlarmReqListener::zmq_init_sck()
{
  ...
  for (retry_cnt=0; retry_cnt < max_retry && rc == -1; retry_cnt++ )
  {
    ...
    if (rc == -1)
    {
      ...
      snmp_log(LOG_ERR, "remove(%s) ignored: %s", sck_file.c_str(), strerror(errno));
    }
  ...
  if (rc == -1)
  {
    // LCOV_EXCL_START
#ifndef UNIT_TEST
    snmp_log(LOG_ERR, "zmq failed to bind %d times - killing myself", retry_cnt);
    kill(getpid(), SIGKILL);
#else
    snmp_log(LOG_ERR, "zmq failed to bind %d times", retry_cnt);
#endif
    // LCOV_EXCL_STOP
    return false;
  }
  
  snmp_log(LOG_ERR, "zmq socket initialized after %d tries", retry_cnt + 1);
